Project: Convert Bash Workflows to "Hatchbox AI" - A Multi-Issue Development Tool
  ...
  `);
  2. Workspace Isolation Benefits:
    - No Git conflicts: Each issue has its own worktree
    - No database conflicts: Each gets a database branch
    - No port conflicts: Automatic port assignment (3000 + issue number)
    - No context confusion: Claude knows exactly which issue it's working on
    - Parallel development: Work on urgent bug while feature is in progress
  3. Claude-Friendly Information Display:
  $ npx hb info 25
